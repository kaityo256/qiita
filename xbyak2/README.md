...
さて、Xbyakでは、作ろうとしている関数のレジスタの値を、作っている関数(Xbyak::CodeGeneratorを継承したクラスのコンストラクタ)の中のローカル変数に取り出すことができない。また、Xbyakから`printf`をcallできないので(多分)、ymmの中身を直接浮動小数点数として表示することは困難だ。
...
組み込み関数を使ってた時には普通にprint文デバッグをしていたのだが、Xbyakからはprintfが(多分)呼べないので、関数の引数経由でモニターするか、デバッガから読むしかなさそう。これ、わりとしんどいと思うんですが、世の中のXbyakerの方々はどうやってデバッグしてるんでしょうか？
